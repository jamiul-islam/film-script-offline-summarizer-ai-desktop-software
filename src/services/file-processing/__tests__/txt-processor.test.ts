...
describe('TxtProcessor', () => {
  ...
  describe('parseFile', () => {
    ...
    it('should handle Windows line endings', async () => {
      ...
      expect(result.content).toBe(content);
      expect(result.metadata.additionalMetadata?.hasWindowsLineEndings).toBe(true);
    ...
    it('should detect very long lines', async () => {
      ...
      expect(result.content).toBe(content);
      expect(result.warnings).toContain('Very long lines detected (max: 1500 characters) - may indicate formatting issues');
    });
    ...
    it('should handle low encoding confidence', async () => {
      ...
      expect(result.warnings?.some(w => w.includes('Low confidence in encoding detection'))).toBe(true);
    });
    ...
    it('should detect potential binary content', async () => {
      ...
      const binaryBuffer = Buffer.from([
        ...
        0x01, 0x02, 0x03, 0x04, // Control characters
        0x57, 0x6F, 0x72, 0x6C, 0x64 // "World"
      ]);
      ...
      expect(result.warnings).toContain('File may contain binary content or use an unsupported encoding');
    });
  ...
  describe('validateFile', () => {
    ...
    it('should warn about potential binary content', async () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.warnings.some(w => w.code === 'POTENTIAL_ENCODING_ISSUE')).toBe(true);
    });
  ...
  describe('calculateConfidence', () => {
    it('should return high confidence for good content', () => {
      const content = 'This is a good text file with substantial content that should give high confidence.';
      const confidence = processor['calculateConfidence'](content, 'utf8', 0.9, 200); // Better ratio
    ...
    it('should return low confidence for short content', () => {
      const confidence = processor['calculateConfidence']('Short', 'utf8', 0.9, 100);
      expect(confidence).toBeLessThan(0.5); // More flexible expectation
    });

    it('should factor in encoding confidence', () => {
      const content = 'Some content';
      const highConfidence = processor['calculateConfidence'](content, 'utf8', 0.9, 100);
      const lowConfidence = processor['calculateConfidence'](content, 'utf8', 0.3, 100);
    ...
    it('should reduce confidence for binary content', () => {
      ...
      const binaryConfidence = processor['calculateConfidence'](binaryContent, 'utf8', 0.9, 100);
      const normalConfidence = processor['calculateConfidence'](normalContent, 'utf8', 0.9, 100);
      expect(binaryConfidence).toBeLessThan(normalConfidence);
    });

    it('should reduce confidence for binary encoding', () => {
      const content = 'Some content';
      const confidence = processor['calculateConfidence'](content, 'binary', 0.5, 100);
      expect(confidence).toBeLessThan(0.5);
    });

    it('should reduce confidence for low text-to-size ratio', () => {
      const content = 'Small';
      const confidence = processor['calculateConfidence'](content, 'utf8', 0.9, 10000);
