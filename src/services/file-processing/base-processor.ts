...
export abstract class BaseFileProcessor implements FileProcessor {
  ...
  abstract parseFile(filePath: string, fileType: FileType): Promise<ParsedScript>;
  ...
  async validateFile(filePath: string): Promise<ValidationResult> {
    ...
    try {
      ...
      if (fileSize > 10 * 1024 * 1024 && fileSize <= this.maxFileSize) { // 10MB warning threshold
        warnings.push({
  ...
  protected extractTitleFromContent(content: string, fallbackTitle: string): string {
  ...
  protected async validateFileContent(content: string): Promise<ValidationWarning[]> {
    const warnings: ValidationWarning[] = [];
    ...
    if (content.includes('�') || content.includes('\uFFFD')) {
      warnings.push({
        ...
        details: 'Detected replacement characters that may indicate encoding problems'
      });
    }
