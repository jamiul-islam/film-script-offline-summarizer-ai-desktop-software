...
export class ResponseParser {
  ...
  private static extractPlotOverview(
  ...
  private static parseCharacterBlock(block: string): Character | null {
    ...
    if (
      lowerBlock.includes('protagonist') ||
      lowerBlock.includes('main character')
    ) {
  ...
  private static extractProductionNotes(
    sections: Record<string, string>,
    warnings: string[]
  ): ProductionNote[] {
    const productionSection =
      sections['production_notes'] || sections['production_challenges'] || '';
  ...
  private static parseProductionNote(text: string): ProductionNote | null {
    ...
    if (
      ...
      lowerText.includes('major cost')
    ) {
      budgetImpact = 'major';
    } else if (
      lowerText.includes('significant cost') ||
      lowerText.includes('substantial')
    ) {
      budgetImpact = 'significant';
    } else if (
      lowerText.includes('minimal cost') ||
  ...
  private static extractBudgetCategory(sections: Record<string, string>) {
    const budgetSection = (
      sections['production_notes'] ||
      sections['general'] ||
      ''
    ).toLowerCase();

    if (
      budgetSection.includes('blockbuster') ||
      budgetSection.includes('major studio')
    ) {
      return 'blockbuster';
    } else if (
      budgetSection.includes('high budget') ||
      budgetSection.includes('expensive')
    ) {
      return 'high';
    } else if (
      budgetSection.includes('medium budget') ||
      budgetSection.includes('moderate cost')
    ) {
      return 'medium';
    } else if (
      budgetSection.includes('low budget') ||
      budgetSection.includes('independent')
    ) {
      return 'low';
    } else if (
      budgetSection.includes('micro budget') ||
      budgetSection.includes('very low cost')
    ) {
      return 'micro';
    }
  ...
  private static extractTargetAudience(
    sections: Record<string, string>
  ): string | undefined {
    const marketSection =
      sections['marketability'] || sections['general'] || '';

    // Look for audience mentions
    const audienceMatch = marketSection.match(
      /(?:target audience|aimed at|appeals to|audience of)\s*:?\s*([^.\n]+)/i
    );
  ...
  private static extractToneAndStyle(
    sections: Record<string, string>
  ): string | undefined {
  ...
  private static extractKeyScenes(
    sections: Record<string, string>
  ): string[] | undefined {
  ...
  private static extractProductionChallenges(
    sections: Record<string, string>
  ): string[] | undefined {
    ...
    for (const line of lines) {
      const trimmedLine = line.trim();
      if (trimmedLine.length > 20) {
        const cleanChallenge = trimmedLine
          .replace(/^[-*•]\s*/, '')
          .replace(/^\d+\.?\s*/, '')
          .trim();
  ...
  private static extractMarketability(
    sections: Record<string, string>
  ): string | undefined {
  ...
  private static parseWithFallback(
    ...
    try {
      ...
      const plotOverview =
        response.substring(0, 500).trim() || 'Summary not available';

      // Try to find character names in the original content
      const characterNames =
        this.extractCharacterNamesFromScript(originalContent);
