      ...
      try {
        ...
        for (const script of existingScripts) {
          try {
            ...
            const parsedScript = await window.electronAPI.file.process(
              script.file_path
            );
            ...
            console.warn(
              `Could not load content for script ${script.title}:`,
              error
            );
      ...
      try {
        for (const file of files) {
          ...
          if (!validation.isValid) {
            ...
            alert(
              `File validation failed for ${file.name}: ${validation.errors.map(e => e.message).join(', ')}`
            );
            continue;
          }
          ...
          try {
            ...
            if (
              ...
              savedScript = existingScripts.find(
                script => script.content_hash === contentHash
              );

              if (savedScript) {
                console.log(
                  `Script "${file.name}" already exists in database, using existing record`
                );
                // Show a friendly message instead of an error
                alert(
                  `"${file.name}" has already been processed. Switching to library view.`
                );
              } else {
          ...
          try {
            ...
            // Trigger confetti animation for successful analysis
              ...
              <Card variant="elevated" className="rounded-xl">
                ...
                <CardContent>
                  <p className="text-slate-300 mb-6">
                    Upload your script files to analyze them with local AI
                    processing. All processing happens on your device for
                    complete privacy.
                  </p>
                  <FileUpload
                  ...
                  {isProcessing && (
                    <div className="mt-4 p-4 bg-blue-900/20 border border-blue-500/30 rounded-lg">
                      <p className="text-blue-300">
                        Processing files and generating summaries... This may
                        take a few moments.
                      </p>
                    </div>
                  )}
            ...
            <div className="max-w-6xl mx-auto">
              <div className="mb-6">
                ...
                <p className="text-slate-400">
                  Manage and organize your uploaded scripts
                </p>
              </div>
              <ScriptLibrary
            ...
            {selectedScript ? (
              ...
              <div className="max-w-6xl mx-auto">
                <div className="mb-6">
                  ...
                  <p className="text-slate-400">
                    View AI-generated summaries of your scripts
                  </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {processedScripts
                    .filter(script => script.summary)
                    .map(script => (
                      <Card
                        key={script.id}
                        variant="hover"
                ...
                {processedScripts.filter(script => script.summary).length ===
                  0 && (
                  <Card>
    ...
    <ThemeProvider defaultTheme="dark" enableAnimations={true}>
      <AppShell currentView={currentView} onNavigate={handleNavigate}>
        ...
        <ConfettiAnimation
          ...
          message={`"${completedScriptTitle}" Analysis Complete! 🎉`}
        />
