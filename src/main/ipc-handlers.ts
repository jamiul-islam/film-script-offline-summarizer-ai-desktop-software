...
export function registerLLMHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (_, operationId: string): Promise<void> => {
      try {
...
export function registerDatabaseHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (
      _,
      scriptData: Omit<DatabaseScript, 'id' | 'createdAt' | 'updatedAt'>
  ...
  ipcMain.handle(
    'db:get-script',
    async (_, scriptId: string): Promise<DatabaseScript | null> => {
      try {
        const db = getDatabaseManager();
  ...
  ipcMain.handle('db:get-all-scripts', async (): Promise<DatabaseScript[]> => {
    try {
      ...
      throw new Error(
        `Failed to get scripts: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  });
  ...
  ipcMain.handle(
    'db:delete-script',
    async (_, scriptId: string): Promise<void> => {
      try {
        const db = getDatabaseManager();
  ...
  ipcMain.handle(
    ...
    async (
      _,
      summaryData: Omit<DatabaseSummary, 'id' | 'createdAt'>
  ...
  ipcMain.handle(
    'db:get-summary',
    async (_, scriptId: string): Promise<DatabaseSummary | null> => {
      try {
        const db = getDatabaseManager();
  ...
  ipcMain.handle(
    ...
    async (
      _,
      evaluationData: Omit<DatabaseScriptEvaluation, 'id' | 'createdAt'>
  ...
  ipcMain.handle(
    'db:get-evaluation',
    async (_, scriptId: string): Promise<DatabaseScriptEvaluation | null> => {
      try {
        const db = getDatabaseManager();
  ...
  ipcMain.handle(
    'db:search-scripts',
    async (_, query: string): Promise<DatabaseScript[]> => {
      try {
        const db = getDatabaseManager();
...
export function cleanupIPCHandlers(): void {
  ...
  ipcMain.removeAllListeners('file:save-dialog');
  ipcMain.removeAllListeners('file:write');
