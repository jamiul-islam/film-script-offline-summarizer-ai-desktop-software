...
describe('DatabaseManager', () => {
  ...
  describe('connection management', () => {
    ...
    it('should handle connection errors gracefully', async () => {
      ...
      await expect(invalidDbManager.connect()).rejects.toThrow(
  ...
  describe('database operations', () => {
    ...
    it('should cascade delete related records', async () => {
      ...
      const summaryCount = db
        .prepare('SELECT COUNT(*) as count FROM summaries WHERE script_id = ?')
        .get(scriptId);
      const evaluationCount = db
        .prepare(
          'SELECT COUNT(*) as count FROM script_evaluations WHERE script_id = ?'
        )
        .get(scriptId);
